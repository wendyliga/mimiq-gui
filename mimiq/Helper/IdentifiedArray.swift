/**
 source: https://github.com/pointfreeco/swift-composable-architecture/blob/main/Sources/ComposableArchitecture/SwiftUI/IdentifiedArray.swift
 */
...
where ID: Hashable {
  ...
  /// Safelly removes and returns the element with the specified identifier.
  ///
  /// - Parameter id: The identifier of the element to remove.
  /// - Returns: The removed element if available.
  @discardableResult
  public mutating func safelyRemove(id: ID) -> Element? {
    let element = self.dictionary[id]
    self.dictionary[id] = nil
    self.ids.removeAll(where: { $0 == id })
    return element
